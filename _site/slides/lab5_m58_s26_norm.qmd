...
subtitle: "due Tuesday Feb 24, 2026"
...
library(infer)      # for the function rep_sample_n()
```
...
```{r}
xqnorm(0.92, 0, 1)  # xqnorm is a function in the mosaic package
xqnorm(0.96, 0, 1)
xqnorm(0.04, 0, 1)
```
...
Note a few things...   1.751 is used instead of 2 for Z* (because we wanted a 92% interval instead of a 95% interval).  And $\hat{p}$ was used instead of p in the SE (because we don't usually know $p$ when making CIs, and we use the closest thing we have, $\hat{p}$).
...
Let's view the first few intervals:
...
```{r}
ci |> 
  summarize(capturecount = sum(capture_p == "yes"))
```
...
Pick a confidence level of your choosing *other than* 92% or 95%. What is the number you will use for multiplying the SE?  (What is Z*?)
