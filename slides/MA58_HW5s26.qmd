---
title: "HW 5 -- Math 58B"
subtitle: "due Thursday, February 26, 2026"
...
(a) What does each point (observation) in each of the figures represent?
(b) Describe how the distribution of the sample proportion, $\hat{p}$, changes as n becomes larger (describe shape, center, and spread).


#### Q2.  Area under the curve

What percent of a "standard" normal distribution [the one with:  N( $\mu = 0; \sigma = 1$)] is found in each region? Be sure to draw a graph [or use a function in R (`xpnorm()` in the **mosaic** package) that provides a graph for you].
...
Identify each of the following statements as TRUE or FALSE.  You don't need to justify it here, but it's always a good idea (read: quiz coming soon!) to practice putting into words why something is true or false.
...
(d) The poll provides evidence (p-value < 0.05) that the percentage of U.S. adults who suffer from chronic illnesses is below 50%.  [You don't need to do the entire hypothesis test here, you should be able to get the answer from the CI.  However, you should also know how to do the entire hypothesis test!]
