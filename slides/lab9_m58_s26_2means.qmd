---
title: 'Lab 9 - Math 58B: Inference on Two Means / Medians'
author: "solutions"
subtitle: "due Tuesday April 7, 2026"
format: pdf
execute:
  warning: false
  message: false
...
* the following two applets may help with forming intuition
     ...
     [http://www.rossmanchance.com/applets/2021/anovashuffle/AnovaShuffle.htm?hideExtras=2](http://www.rossmanchance.com/applets/2021/anovashuffle/AnovaShuffle.htm?hideExtras=2)
 
     - Bootstrapping applet:  
     [http://www.rossmanchance.com/applets/2021/twoboot/TwoBoot.html?hideExtras=2](http://www.rossmanchance.com/applets/2021/twoboot/TwoBoot.html?hideExtras=2)
...
Try `geom_point()`, `geom_boxplot()`, `geom_jitter()`, `geom_bar()`,...
...
# check out geom_jitter.... just like geom_point with a little bit of noise added!
# in the geom_jitter you can adjust `width = ...` or `height = ...`

births14 |>
  ggplot(aes(x = premie, y = habit, color = marital)) + 
  geom_jitter()
```
...
null_wks |>
  ...
  geom_vline(xintercept = wks_obs) + 
  labs(x = "null differences in mean weeks")

null_wks |>
  summarize(p_val = 2 * mean(stat <= wks_obs))
```
...
births14 |>
  specify(weeks ~ mature) |>
  generate(reps = 1000, type = "bootstrap") |>  # no hypothesize step!!!
  calculate(stat = "diff in means", order = c("mature mom", "younger mom")) |>
  visualize() +
...
Using the distributions above, the CI for $\mu_Y - \mu_M$ can be taken directly from the bootstrap sampling distributions.

We are 93% confident that the true difference in average weeks of gestation between mature versus younger moms is between -0.524 weeks and 0.227 weeks.  Note that because the confidence interval overlaps zero, we know that the value of zero is a plausible parameter.
...
ci_diff_93 <- boot_diff |>
  summarize(lower = quantile(stat, probs = 0.035),
            upper = quantile(stat, probs = 0.965))
...
boot_diff |>
  ...
  geom_vline(xintercept = ci_diff_93$lower, color = "red") + 
  geom_vline(xintercept = ci_diff_93$upper, color = "red") + 
  labs(x = "bootstrap differences in mean weeks")
```
...
### Q3. Randomization Test for the difference in means
...
### Q4. Randomization Test for the difference in medians
...
### Q5. Bootstrap Confidence Interval for the difference in means
...
### Q6. Bootstrap Confidence Interval for the difference in medians
