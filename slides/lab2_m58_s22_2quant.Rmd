...
`responsevariable ~ explanatory variable`
...
#### Q2. 
Create a scatterplot with life expectancy on the y-axis and fertility rate on the x-axis.  Color the points based on their continent. Change the x and y axis labels to be more descriptive.

    
#### Q3. 
Make two graphs which are similar to the plot in the previous questions.  First, use only observations from 1962 by `filter()`ing on `year`. Make a second scatterplot for observations from 2003.  (It is possible, and fun!, to use `filter()` to get both years and then to apply `facet_wrap()` to create side-by-side plots.)

Name one difference and one similarity in the pair of plots from 1962 and 2003.


#### Q4. 
On each of the separate plots (1962 and 2003), add the linear model to the plot (separate linear model for each continent).

Also add (to each of the 1962 and 2003 plots separately) a single linear regression which is fit on all countries together simultaneously.  Color the overall regression line black.
...
#### Q6. 
Calculate the intercept and slope describing the least squares regression model regressing life expectancy (the response variable) on fertility (the explanatory variable).  Use 2003 and all continents.  Write down the full model (hint: when writing the model down, either use a hat or add a residual).
