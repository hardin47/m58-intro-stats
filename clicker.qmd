...
154.  If there is no relationship in the population (true correlation  = 0), then r=0.
...
155.  If there is no relationship in the population (true slope 1 = 0), then b1=0.
...
156. A smaller variability around the regression line (can be thought of as: σ or MSE or variability of the ei):

(a) increases the variability of b1.
(b) decreases the variability of b1.
(c) doesn’t necessarily change the variability of b1.

151. A smaller variability in the explanatory variable (SD(X) = sx):

(a) increases the variability of b1.
(b) decreases the variability of b1.
(c) doesn’t necessarily change the variability of b1.

157.  A smaller sample size (n):
(a) increases the variability of b1.
(b) decreases the variability of b1.
(c) doesn’t necessarily change the variability of b1.
...
158.  The regression technical assumptions include:
...
159. The technical conditions do not include:
...
160.  What happens if the technical conditions are not met?
	...
	The distribution of b1 is wrong (therefore incorrect p-values and CI).
	The computer (R) will produce an error when running the linear model.

161. Which linear regression condition is violated?
...
162. Which linear regression condition is violated?
...
(d) normal errors
(e) outliers
163. Which linear regression condition is violated?
...
(d) normal errors
(e) outliers
164. Which of the below correctly describes the roles of variables in this regression model?
Est 			SE 		t value 	Pr(>|t|)
(Intercept) 	197.96 		59.20 	3.34     0.0058
volume 	0.71 		0.06 	11.67 	0.0000
coverpb 	-184.05 	40.49 	-4.55 	0.0007
(a) response: weight 
...
165. An econometrician is interested in evaluating the relation of demand for building materials to mortgage rates in LA and SF. 
Y ̂ = 10 + 5X1 + 8X2
where	
X1 = mortgage rate in % 
X2 = 1 if SF, 0 if LA
...
X2 = 1 if SF, 0 if LA
Y = demand in $100 per capita
the effect of living in LA rather than SF is a ________ demand by an estimated ________ holding the effect of mortgage rate constant.
...
167. Consider the housing model, 
Y ̂= 12.2 + 0.000468⋅sqft − 0.0603⋅# bedrooms

the coefficient on bedrooms (− 0.0603) can be interpreted as the change in predicted ln(price)…
...
168. To test if there is convincing evidence that the slope of the regression line between ln(price) and square feet (only, no bedroom here) is different from zero, 
...
p-value = probability of observed data (b1) or more extreme if H0 is true (β1 = 0). 
169. With # bedrooms in the model, 
Y ̂= 12.2 + 0.000468⋅sqft − 0.0603⋅# bedrooms
the test is now:
H0: β1 = 0
Ha: β1 ≠ 0
In words, the test asks if there is convincing evidence that:
	...
	adding square footage to your house causes the value to increase
...
170. We created a 95% confidence interval for the mean GPA given 10 absences to be (3.20, 3.42).  What is the correct interpretation?
...
171. We created a 95% prediction interval for an individual GPA given 10 absences to be (3, 3.62).  What is the correct interpretation?
...
Ok: there is a 95% chance that if I randomly select someone with 10 absences, their GPA will be between 3 and 3.62. 




172. Prediction intervals and confidence intervals have the same technical conditions:
(a) TRUE
(b) FALSE
(c) sort of
...
173.  It is often a good idea to transform the variable(s)…
...
174. A large value of R2 says…
	the technical conditions hold.
	the variability in the response is well explained by the explanatory variables.
	the explanatory variables determine the response variable.  
	the explanatory variables are significant. 
```
