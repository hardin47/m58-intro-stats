...
(@) Let's say you take a random sample and compute $\hat{p}=0.3.$  After bootstrapping, you see that the bootstrapped resamples produce almost all the $\hat{p}_{boot}$ within plus or minus 0.01 of your original statistic. It seems that the parameter $p$ is probably:[^46]
	   a. 0.3
	   b. between (0.2, 0.4)
	   c. between (0.29, 0.31)
	   d. between (0.28, 0.32)
...
(@) In a second study, I set out to obtain twice as much data (as in the first study) in order to create a 95% CI for the true proportion $p.$ What is the impact (of the larger sample) on the CI?[^48]
    (a) narrower
