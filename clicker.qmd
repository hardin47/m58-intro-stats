...
(@) The sum of residuals from the sample mean (no X):[^22] 
...
(@) What math is used to find the value of $m$ that minimizes:[^25]
$$\sum_{i=1}^n(Y_i - m)^2$$
    (a) combinatorics
    (b) derivative
    (c) integral
...
(@) If communication medium and cheating are independent variables, how many of the email senders (out of 26) would you expect to cheat?[^35]
    (a)	10 (ish)
    (b)	13 (ish)
    (c)	16 (ish)
    (d)	20 (ish)
...
(@) When looking at the null differences, is the observed result of 28.7%:[^36]
    (a)	Very surprising
    (b)	Somewhat surprising
    (c)	Not very surprising
...
(@) Hypothesis: the number of hours that grade-school children spend doing homework predicts their future success on standardized tests.[^37]
    (a) null, one sided
    (b) null, two sided
...
(@) Hypothesis: king cheetahs on average run the same speed as standard spotted cheetahs.[^38]
    (a) null, one sided
    (b) null, two sided
    (c) alternative, one sided
    (d) alternative, two sided
...
(@) Hypothesis: the mean length of African elephant tusks has changed over the last 100 years.[^39]
    (a) null, one sided
    (b) null, two sided
    (c) alternative, one sided
    (d) alternative, two sided
...
(@) Hypothesis: the risk of facial clefts is equal for babies born to mothers who take folic acid supplements compared with those from mothers who do not.[^40]
    (a) null, one sided
    (b) null, two sided
    (c) alternative, one sided
    (d) alternative, two sided
...
(@) Hypothesis: caffeine intake during pregnancy affects mean birth weight.[^41]
    (a) null, one sided
    (b) null, two sided
    (c) alternative, one sided
    (d) alternative, two sided
...
(@) People check-in
    (a) So far, so good
    (b) I can go to office hours / mentor sessions, but I didn't go this week.
...
(@) I know where to find: the solutions to the worksheets, the clicker questions (with solutions), and the HW/Lab solutions[^43]
     (a) TRUE
     (b) FALSE
     
[^43]: The worksheet solutions and clicker questions are on the main course website.  The HW & Lab solutions are on Canvas under Files.
    
---

(@) You have a sample of size n = 50.  You sample with replacement 1000 times (to get 1000 bootstrap resamples). What is the sample size of each bootstrap resample?[^44]
    (a) 50
    (b) 1000
...
(@) You have a sample of size n = 50.  You sample with replacement 1000 times (to get 1000 bootstrap resamples). How many bootstrap statistics will you have?[^45] 
     (a) 50
     (b) 1000
...
(@) In this class, the word parameter means:[^42]
...
(@) First study: Let's say you take a random sample and compute $\hat{p}=0.3.$  After bootstrapping, you see that 95% of the bootstrapped resamples ($\hat{p}_{boot}$) are within plus or minus 0.01 of your original statistic ($\hat{p}$). It seems that the parameter $p$ is probably:[^46]
	   a. 0.3
	   b. between (0.2, 0.4)
	   c. between (0.29, 0.31)
	   d. between (0.28, 0.32)
...
(@) In a second analysis, I create a 90% CI for the true proportion $p.$ What is the impact (of switching from 95% to 90%) on the CI?[^47]
    (a) narrower
...
(@) In a third study, I set out to obtain twice as much data (as in the first study) in order to create a 95% CI for the true proportion $p.$ What is the impact (of the larger sample) on the CI?[^48]
    (a) narrower
...
(@) What is one main reason to use bootstrapping to find a confidence interval?[^49]
...
(@) 95% CI for the true median mercury:[^50]
...
(@) What are the observational units for your individual candy study?[^51]
...
(@) What are the observational units for the class compilation (dotplot)?[^52]
     a. Color of the candy
     b. Piece of candy
     c. Cup of candy
     d. The Hershey Company
...
(@) How does the sampling distribution for the sample proportion change as n changes (for a fixed p)?[^53]
...
(@) How does the sampling distribution change as p changes (for a fixed n)?[^54]
     a. The spread changes
     b. The symmetry changes
     c. The center changes
     d. The shape changes
...
(@) The Central Limit Theorem says that the distribution of $\hat{p}$ will be approximately normal with what center:[^55]
    (a) $\hat{p}$
    (b) $p$
    (c) 0.5
    (d) 1
...
(@) Would you rather have an extra 20 points on the SAT or an extra 10 points on the ACT?[^56]
     a. +20 on the SAT
     b. +10 on the ACT
...
(@) The standardized score (z-score) counts:[^57]
...
(@) If the normal distribution is a good model, we would expect the large majority of our z scores to be:[^58]
...
(@) With your cup of candy, you personally got a Z score of:[^59]
...
(@) Assume n = 100 and p= 0.8  (note: $\sqrt{(0.8 \cdot 0.2)/100} = 0.4/10 = 0.04$) Which statement is true?[^61]
...
(@) If you want a 90% confidence interval for p, your z* multiplier should be[^62]
     (a) less than 1
...
(@) What is the difference between Z* and a Z score?[^63]
...
(@) Let's say we are making confidence intervals (not doing a hypothesis test), what is your best guess for $SE(\hat{p})$?[^64]
...
(@) The following is a correct interpretation of the CI:[^65]  
...
[^65]: a. FALSE.  We are 100% confident that the interval contains the sample proportion! The statement should be "95% confident that the interval includes the population proportion who believe that the global poverty rate has doubled."

---

(@) The following is a correct interpretation of the CI:[^66]  
...
(@) Let's say that the null hypothesis (e.g., p=0.47) is TRUE.   I reject $H_0$ if the p-value < 0.03. How often will I reject the null hypothesis?[^67]
     (a) 1 % of the time
     (b) 3% of the time
     (c) 5 % of the time
     (d) 95% of the time
...
(@) Let's say that the null hypothesis (e.g., p=0.47) is TRUE.   I reject $H_0$ if the p-value < 0.03. How often will p be in a 97% confidence interval?[^68]
    (a) 1 % of the time
    (b) 3% of the time
    (c) 5 % of the time
    (d) 95% of the time
...
(@) Suppose the sample is 10 times larger.  The SE of the statistic:[^69]
     a. increases
     b. stays the same
     c. decrease
...
(@) Suppose the population is 10 times larger.  The SE of the statistic:[^70]
     a. increases
     b. stays the same
     c. decrease
...
(@) Suppose the sample is 10 times larger.  The variability of the data:[^71]
     a. increases
     b. stays the same
     c. decrease
...
(@) How many hits out of 20 at bats would make you believe him?[^72]
    (a)	5
    (b)	6
    (c)	7
    (d)	8
...
(@) Power is the probability that:[^75]
    (a)	We give him a raise when he deserves it.
    (b)	We don't give him a raise when he deserves it.
    (c)	We give him a raise when he doesn't deserve it.
    (d)	We don't give him a raise when he doesn't deserve it.
...
(@) The player is more worried about[^76]
    (a)	A type I error
    (b)	A type II error
...
(@) The manager is more worried about[^77]
    (a)	A type I error
    (b)	A type II error
...
(@) Increasing your sample size[^78]
...
(@) Making your discernibility level more stringent ($\alpha$ smaller)[^79]
    (a)	Increases your power
    (b)	Decreases your power
...
(@) Alien example (see notes 4.6.1): Is the Alien's interval for the true proportion of all humans who self-identify as female consistent with your lived experience?[^81]
    (a) Yes
    (b) No
...
(@) Alien example (see notes 4.6.1): As we've seen with the applet, about 5% of all 95% intervals fail to capture the actual value of the population parameter.  Do you think the alien just got a "red" interval?[^82]
    (a)	Yes
    (b)	No
...
(@) Alien example (see notes 4.6.1): Would it be reasonable for the alien to conclude, with 95% confidence, that between 17.4% and 34.6% of US Senators in the year 2026 self-identify as female?[^83]
     (a)	Yes
     (b)	No

[^83]: b. No. We know (for sure, with 100% confidence) that exactly 26% of U.S. senators in 2026 self identify as female. If that's the entire population of interest, there's no reason to calculate a confidence interval.

---

(@) Gettysburg example (see notes 4.6.2): As we've seen with the applet, about 5% of all 95% intervals fail to capture the actual value of the population parameter.  Do you think you just got a "red" interval for proportion of short words?[^82a]
    (a)	Yes
    (b)	No

[^82a]: b. No.  You didn't just "get unlucky".  Instead, the reason the interval failed to capture the true parameter is because the sample was not representative of the population. There was not a simple random sample.


---

(@) The "random" part in clinical trials typically comes from:[^84]
    (a) random samples
...
(@) The "random" part in polling typically comes from:[^85]
    (a) random samples
    (b) random allocation of treatment
...
(@) You want to collect data to investigate whether teenagers in the United States have read fewer Harry Potter books than teenagers in the United Kingdom.  Would you make use of random sampling, random assignment, both, or neither?[^86]
    (a)	Random sampling
    (b)	Random assignment
    (c)	Both
    (d)	Neither
...
(@) An instructor wants to investigate whether using a red pen to grade assignments leads to lower scores on exams than using a blue pen to grade assignments.  Would you advise the professor to make use of random sampling, random assignment, both, or neither?[^87]
    (a)	Random sampling
    (b)	Random assignment
    (c)	Both
    (d)	Neither
...
(@) A student decides to investigate whether NFL football games played in indoor stadiums tend to have more points scored than games played outdoors.  The student examines points scored in every NFL game of the 2022 season. Has the student used random sampling, random assignment, both, or neither?[^88]
    (a)	Random sampling
    (b)	Random assignment
    (c)	Both
    (d)	Neither
...
(@) In order to find a CI for the true RR, our steps are:[^90]  
Step 1. ln(RR-hat)  
...
(@) In finding a CI for $p_1$/$p_2$, why is it okay to exponentiate the end points of the interval for ln($p_1$/$p_2$)?[^91] 
...
(@) Usually, the CI for  $p_1$/$p_2$ is considered to be "discernible" if[^92] 
...
(@) In order to find a CI for the true OR, our steps are:[^93]  
Step 1. ln(OR-hat)  
...
(@) Sample 100 people who are in the NBA and 100 people who are not in the NBA. Record if the person is over 6' tall. What is measurable?[^95]
     (a) P(NBA if 6' tall)
     (b) P(6' tall if in the NBA)
     (c) both
     (d) neither  
...
(@) When we randomly select individuals based on the explanatory variable, we cannot accurately measure[^97]
...
(@) The odds ratio is invariant to which variable is explanatory and which is response means:[^98]
...
(@) One reason we should be careful interpreting relative risks is if:[^99]
...
(@) If the null hypothesis is true, the observed counts will equal the expected counts.[^100]
     (a) True
     (b) False
...
(@) To reject the null hypothesis we want to see[^101]
     (a) a small $X^2$ value
     (b) a big $X^2$ value
...
(@) For the lighting study, which variable is the explanatory variable?[^103]
     (a) sleeping light
     (b) eye sightedness
     (c) child
     (d) parent
...
(@) If we sample randomly from a population, the conclusions we can make are about:[^104]
     (a) causation 
...
(@) Based on the night light / myopia example, the correct conclusion is:[^105]
...
(@) A possible confounding variable for the night light study is:[^106]
     (a) low birth weight
...
(@) Which dataset has the smallest standard deviation?[^107]
     (a)	A: left
     (b)	B: center
     (c)	C: right
...
(@) Which of the two dotplots displays the dataset with the smaller IQR?[^108]
     (a)	A
     (b)	B
...
(@) The standard deviation of weights (mean = 167 lbs) is approximately[^109]
     (a) 1
     (b) 5
     (c) 10
     (d) 35
...
(@) The standard deviation of average weights (mean = 167 lbs) in repeated samples of size 10 is approximately[^110]
     (a) 1
     (b) 5
     (c) 10
     (d) 35
...
(@) The standard deviation of average weights (mean = 167 lbs) in repeated samples of size 50 is approximately[^111]
     (a) 1
     (b) 5
     (c) 10
     (d) 35
...
(@) The standard deviation of average weights (mean = 167 lbs) in repeated samples of size 1000 is approximately[^112]
     (a) 1
     (b) 5
     (c) 10
     (d) 35
...
(@) The sampling distribution of the mean will be[^113]
...
(@) The sampling distribution of the mean will be[^114]
...
(@) Why did we switch from talking about total weight to talking about average weight?[^115]
...
(@) When the population is skewed right, the sampling distribution for the sample mean will be[^116]
...
(@) What type of variable is "healthy body temp"?[^118]
     a. explanatory
     b. response
...
(@) We use $s$ instead of $\sigma$ because[^119]
...
(@) The variability associated with $\overline{X}$ is[^120]
...
(@) If asked to "determine how many standard errors the sample mean (98.249) falls from the hypothesized value of 98.6", which formula should you use?[^121]
...
(@) When we use $s$ instead of $\sigma$ in the CI for $\mu$, but still keep z* (instead of using a t* multiplier), the resulting CI has coverage[^122]
...
(@) What is the difference between t* and a t score?[^123]
...
(@) What is the correct interpretation of the 92% CI for $\mu$ which is given as (98.2, 98.3)?[^124]
...
(@) Let's say you *truly* believe that the true average body temp is between (98.2, 98.3). (Your CI is green.) You record a temp of 98.6 F.  Do you think you are sick?[^125]
...
(@) The variability associated with $\overline{X}$ is[^126]
      (a) less than the variability of X
      (b) more than the variability of X
      (c) the same as the variability of X
      (d) unrelated to the variability of X
...
(@) The variability associated with predicting a new value, $X_{n+1}$,[^127]
...
(@) Where should a prediction interval for a new value, $X_{n+1}$, be centered?[^129]
      (a) $\overline{X}$
      (b) $\mu$
      (c) 98.6
...
(@) What is the correct interpretation of the 95% PI for $X_{n+1}$ which is given as (96.79, 99.70)?[^130]
...
[^131]: b. stricter technical conditions than CIs, becausse the CLT does not apply.

---

(@) When the population is skewed right, the sampling distribution for the sample mean will be[^132]
      (a) always skewed right
      (b) skewed right if n is big enough
      (c) always normal
      (d) normal if n is big enough
...
(@) When the population is skewed right, the distribution for the data will be[^133]
      (a) always skewed right
      (b) skewed right if $n$ is big enough
      (c) always normal
      (d) normal if $n$ is big enough
...
(@) A newspaper article claims that the average age for people who receive food stamps is 40 years. You believe that the average age is lower. You take a random sample of 100 people who receive food stamps and find their average age to be 39.2 years. You find that 39.2 is discernibly lower than the age of 40 stated in the article (p < 0.05). What would be an appropriate interpretation of the result?[^134]
      (a) The statistically discernible result indicates that the majority of people who receive food stamps is younger than 40.
      (b) Although the result is statistically discernible, the difference in age is not of practical importance.
      (c) An error must have been made. This difference is too small to be statistically discernible.

[^134]: b. Although the result is statistically discernible, the difference in age is not of practical importance.

---

(@) In order to investigate a claim that the average time required for the county fire department to respond to a reported fire is greater than 5 minutes, county staff determined the response times for 40 randomly selected fire reports.  The data were used to test $H_0: \mu = 5$ versus $H_a:  \mu > 5$, and the computed p-value was 0.12.  If a 0.05 level of discernibility is used, what conclusions can be drawn?[^135]
...
(@) You have two samples of size n = 50.  You sample with replacement 1000 times (to get 1000 bootstrap resamples). What is the sample size of each bootstrap resample?[^136]
     (a) 50
     (b) 1000
...
(@) You have two samples of size n = 50.  You sample with replacement 1000 times (to get 1000 bootstrap resamples). How many bootstrap statistics will you have?[^137]
     (a) 50
     (b) 1000
...
(@) You have two samples of size n = 50.  You shuffle the explanatory and response variables (i.e., sample without replacement) 1000 times. What is the sample size of each group after shuffling?[^138]
     (a) 50
     (b) 1000
...
(@) You have two samples of size n = 50.  You shuffle the explanatory and response variables (i.e., sample without replacement) 1000 times. How many randomization statistics will you have?[^139]
     (a) 50
     (b) 1000
...
(@) We typically compare means instead of medians because[^140]
...
(@) $SE(\overline{X}_1 - \overline{X}_2)$ is:[^141]
...
(@) The distribution of age of death is:[^142]
     a. right skewed
     b. left skewed
     c. symmetric
...
(@) The standard deviation for the age of death is likely around:[^143]
     (a) 1 year
     (b) 10 years
     (c) 20 years
     (d) 50 years
...
(@) The number of left handed people is likely:[^144]
     (a) 10
     (b) 100
     (c) 300
     (d) 500
...
(@) Are the two samples (lefties and righties) independent?[^145]
     (a) yes
     (b) no
     (c) we can't tell
...
(@) For the handedness example, which has a lower p-value?[^146]
      a. Scenario 2
      b. Scenario 3
...
(@) For the handedness example, which has a lower p-value?[^147]
      a. Scenario 3
      b. Scenario 4
...
(@) How does each affect the power?[^148]
...
(@) We use the t-distribution (instead of the z-distribution) because:[^149]
...
(@) If we use the SE and the z-curve (instead of t-curve) to find the p-value (assuming $\overline{X}$ values are reasonably different):[^150]
...
(@) If we use the SE and the z-curve (instead of t-curve) to find a 95% CI:[^151]
...
(@) What is the primary reason to use a matched pairs design?[^151a]
      a. To increase the sample size
      b. To ensure that not everyone is assigned to the treatment group.
      c. To reduce the variability
      d. To find a better estimate of $\mu_1 - \mu_2$

[^151a]: c. To reduce the variability
---

(@) A farmer wants to see whether referring to cows by name increases their milk production. He selects half of his cows at random, gives them names, and frequently calls them by name. The other half of his cows he does not call by name. Then he measures the milk production of each cow over a one-week period.[^151b]
      a. paired
      b. independent
...
(@) A farmer wants to know whether hand-milking or machine-milking tends to produce more milk from cows. He examines records of how much milk the cows have produced in the past, and orders them from most to least productive. For the top two milk producers, randomly assign one to hand-milking and the other to machine-milking. Do the same for the next two and the next two and so on.[^151c]
      a. paired
      b. independent
...
(@) You wonder whether students at your school tend to drive newer cars than faculty at your school. You take a random sample of 20 students and a random sample of 10 faculty members, and ask each person how old their car is.[^151d]
      a. paired
      b. independent
...
(@) In the ANOVA setting, the null hypothesis is always: $$H_0: \mu_1 = \mu_2 = \mu_3 = \ldots = \mu_I$$
What is the alternative hypothesis?[^152]
     ...
     (d) $H_a$: at least one $\mu_i$ is discernibly different
...
(@) In order to tell whether the differences in sample means are discernible, we need to ALSO know:[^153]
...
(@) We reject the null hypothesis if:[^155]
...
(@) What types of values will the F-ratio have when the null hypothesis is false, that is, when the population means are not all equal?[^156]
     (a) large, positive
     (b) large, negative
     (c) small, positive
     (d) small, negative
...
(@) With ANOVA, if the null hypothesis is true, then[^157]
...
(@) How can I figure out which $\mu_i$ is different?[^158]
...
(@) Consider a categorical variable with 4 levels. In addition to the intercept how many variables show up in the linear model output?[^159]
      a. 1
      b. 3
      c. 4
      d. n-4
...
(@) If there is no relationship in the population (true correlation $\rho = 0$), then $r=0.$[^160]
     (a) TRUE
     (b) FALSE
...
(@) If there is no relationship in the population (true slope $\beta_1 = 0$), then $b_1=0.$[^161]
     (a) TRUE
     (b) FALSE
...
(@) If we set a parameter equal to XXXX, should we expect the statistic to be XXXX?[^161a]
     (a) Yes
     (b) No

[^161a]: No. Because statistics vary from sample to sample, always.

---

(@) A smaller variability around the regression line (can be thought of as: $\sigma$ or MSE or variability of the $e_i$):[^162]
...
(@) A smaller variability in the explanatory variable (SD(X) = $s_x$):[^163]
     (a) increases the variability of $b_1.$
     (b) decreases the variability of $b_1.$
     (c) doesn't necessarily change the variability of $b_1.$
...
(@) The regression technical assumptions include:[^165]
...
[^165]: c. The residuals are normally distributed.  (b. is not true as there are no technical conditions on X.  d. and e. are both a result of either a. or c. being true. a. The Y variable is normally distributed is only true if we add "at each X".) 

---

(@) The technical conditions do not include:[^166]
     (a) normally distributed residuals
     (b) normally distributed response at each X
...
(@) What happens if the technical conditions are not met?[^167]
...
(@) Which linear regression condition is violated?[^169]
     (a) linearity
     (b) constant errors
     (c) independent errors
     (d) normal errors
...
(@) Which linear regression condition is violated?[^170]
     (a) linearity
     (b) constant errors
     (c) independent errors
     (d) normal errors
...
(@) It is often a good idea to transform the variable(s)...[^180]
...
(@) We created a 95% confidence interval for the mean GPA given 10 absences to be (3.20, 3.42).  What is the correct interpretation?[^177]
...
(@) We created a 95% prediction interval for an individual GPA given 10 absences to be (3, 3.62).  What is the correct interpretation?[^178]
...
(@) Prediction intervals and confidence intervals have the same technical conditions:[^179]
     (a) TRUE
     (b) FALSE
     (c) sort of
...
(@) Which of the below correctly describes the roles of variables in this regression model?[^171]
...
(@) Holding constant the city, additional 1% of mortgage rate would predict average ________ in the mean demand.[^172] 
$$\hat{Y} =  10 + 5X_1 + 8X_2$$
...
[^172]: a. predicted $500 more per capita
...
(@) Consider the housing model, (Y = ln price)
$$\hat{Y} = 12.2 + 0.000468 \cdot \mbox{sqft} − 0.0603\cdot \# \mbox{bedrooms}$$
coef on bedrooms (-0.0603) is change in pred ln(price) ...[^174]
...
(@) To test if there is convincing evidence that the slope of the regression line between ln(price) and square feet (only, no bedroom here) is different from zero, what are the appropriate hypotheses?[^175]
     (a)	$H_0: b_0 = 0$
      	  $H_a: b_0 \ne 0$
     (b) 	$H_0: b_1 = 0$
      	  $H_a: b_1 \ne 0$
...
$$\hat{Y} = 12.2 + 0.000468 \cdot \mbox{sqft} − 0.0603\cdot \# \mbox{bedrooms}$$
      ...
      c. adding sq ft to your house causes the value to increase
...
(@) An interaction term in a multiple regression model may be used when:[^184]
    ...
    d. the relationship between $X_1$ and $Y$ changes for differing values of $X_2$.
    e. $X_1$ and $X_2$ are correlated.

[^184]: d. the relationship between $X_1$ and $Y$ changes for differing values of $X_2$.

---

(@) When comparing Model 1 with $X_1$ versus Model 2 with $X_1$ and $X_2$, consider SSE = $\sum_i(Y_i - \hat{Y}_i)^2$.[^185]
    a. SSE$_1$ is always bigger than (or equal to) SSE$_2$
    b. SSE$_1$ is always less than (or equal to) SSE$_2$
    c. SSE$_1$ is always the same as SSE$_2$
    d. SSE$_1$ may be bigger or may be smaller than SSE$_2$
    
[^185]: a. SSE$_1$ is always bigger than (or equal to) SSE$_2$

---

(@) A large value of $R^2$ says...[^181]
      ...
      d. the explanatory variable(s) are discernible.

[^181]: b. the variability in the response is well explained by the explanatory variable(s).

---

(@) $R^2$ for the regression line for predicting GPA based on absences is 91.31. The interpretation is that 91.31% of[^182]
     a. GPAs can be accurately predicted by absence.
     b. variability in predictions of GPA is explained by absence.
     c. variability in predictions of absences is explained by GPA.
     d. variability in GPA is explained by absences.
     e. variability in absences is explained by GPA.
     
[^182]: d. variability in GPA is explained by absences.

---

(@) The adjusted $R^2$ is "adjusted for" the:[^183]
    a. number of predictors only.
    b. sample size only.
    c. number of predictors and the sample size.
    d. None of the above.

[^183]: c. number of predictors and the sample size.

---

(@) In a multiple regression model, which of the following is correct regarding the value of the adjusted $R^2$?[^184]
    a. It can be negative.
    b. It has to be positive.
    c. It has to be larger than the coefficient of multiple determination $(R^2).$
    d. It can be larger than 1.

[^184]: a. It can be negative. If the model fits so poorly that it explains less variance than a simple horizontal line (the mean-only model), the adjusted $R^2$ will be negative. This can happen when you have too many useless predictors relative to sample size. (c) and (d) are wrong — adjusted $R^2$ is always $\leq R^2$, never larger, and it can't exceed 1.
