...
56. The standardized score (z-score) counts:[^56]
...
[^56]: a. the number of standard deviations from the mean

---

57. If the normal distribution is a good model, we would expect the large majority of our z scores to be:[^57]
...
58. With your cup of candy, you personally got a Z score of:[^58]
    (a) between (-1, 1) (not including 1)
    (b) between (-2, -1] or [1, 2)
    (c) between (-3, -2] or [2, 3)
    (d) -3 or smaller or 3 or above

[^58]: a. or b.  you most likely got between -2 and 2

---

59. Assume n = 100 and p= 0.8  (note: $\sqrt{(0.8 \cdot 0.2)/100} = 0.4/10 = 0.04$)  
What is the largest reasonable distance between $\hat{p}$ and $p$?  
That is, we would expect $\hat{p}$ and $p$ to be no more than _____ apart[^59]
    (a) 0.04
    (b) 0.08
    (c) 0.12
    (d) 0.16
...
[^59]: b. 0.08 (we usually consider two standard deviations)

---


60. Assume n = 100 and p= 0.8  (note: $\sqrt{(0.8 \cdot 0.2)/100} = 0.4/10 = 0.04$) Which statement is true?[^60]
...
[^60]: b. 95% of $\hat{p}$ are between (0.72, 0.88)

---

61.  If you want a 90% confidence interval for p, your z* multiplier should be[^61]
     (a) less than 1
...
[^61]: b. less than 2 (but greater than 1)

---

62. What is the difference between Z* and a Z score?[^62]
...
[^62]: a. Z score comes from the data, Z* and is a pre-defined unit of measurement.

---

63. Let’s say we are making confidence intervals (not doing a hypothesis test), what is your best guess for $SE(\hat{p})$?[^63]
...
[^63]: c. $\sqrt{\hat{p} \cdot (1 - \hat{p}) / n}$

---

64. The following is a correct interpretation of the CI:[^64]  
...
[^65]: b. FALSE (we are 95% confident that the new interval will contain the true value.  We do not think that the new interval will be the same as the original interval.)

---

66.  Let’s say that the null hypothesis (e.g., p=0.47) is TRUE.   My level of significance is 0.03 (reject if p-value < 0.03). How often will I reject the null hypothesis?[^66]
     (a) 1 % of the time
     (b) 3% of the time
     (c) 5 % of the time
     (d) 95% of the time
...
67. Let’s say that the null hypothesis (e.g., p=0.47) is TRUE.   My level of significance is 0.03.  
How often will p be in a 97% confidence interval?[^67]
    (a) 1 % of the time
    (b) 3% of the time
    (c) 5 % of the time
    (d) 95% of the time
...
68.  Suppose the sample is 10 times larger.  The SE of the statistic:[^68]
     a. increases
     b. stays the same
     c. decrease
...
69.  Suppose the population is 10 times larger.  The SE of the statistic:[^69]
     a. increases
     b. stays the same
     c. decrease

[^69]: b. stays the same (the population size has no effect on the sampling distribution of the statistic)

---

70. Suppose the sample is 10 times larger.  The variability of the data:[^70]
     a. increases
     b. stays the same
     c. decrease

[^70]: b. stays the same (the variability of the data should be the same as the variability of the population, regardless of the sample size)
